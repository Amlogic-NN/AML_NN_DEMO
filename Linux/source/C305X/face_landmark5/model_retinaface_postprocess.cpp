...
static float bbox_32[12][20][8]; //384/32,640/32
static float bbox_16[24][40][8];
static float bbox_8[48][80][8];
//prob score,input
static float prob_32[240][2][2];
static float prob_16[960][2][2];
static float prob_8[3840][2][2];
//land mark
static float land_32[12][20][20]; //384/32,640/32
static float land_16[24][40][20];
static float land_8[48][80][20];
...
static int g_detect_number = 230;

static float retina_overlap(float x1, float w1, float x2, float w2)
{
    float l1 = x1;
    float l2 = x2;
...
static float retina_box_intersection(box a, box b)
{
    float area = 0;
...
static float retina_box_union(box a, box b)
{
...
static float retina_box_iou(box a, box b)
{
    return retina_box_intersection(a, b)/retina_box_union(a, b);
}

static int retina_nms_comparator(const void *pa, const void *pb)
{
...
static void do_global_sort(box *boxe1,box *boxe2, float prob1[][1],float prob2[][1], int len1,int len2,float thresh)
{
    int i,j;
    for (i = 0; i < len1; ++i)
    {
...
static void retina_do_nms_sort(box *boxes, float probs[][1], int total,  float thresh)
{
    int i, j;
...
static void retina_result(int num, float thresh, box *boxes, float probs[][1],landmark *pland,face_detect_out_t* pface_det_result)
{
    int i;
...
static void retina_point5_result(int num, float thresh, box *boxes, float probs[][1],landmark *pland,face_landmark5_out_t* pface_landmark5_result)
{
    int i,j;
...
static int face_detect_postprocess(face_detect_out_t* pface_det_result )
{
    int i = 0,j = 0,k = 0,x,y;
    int idx;
...
static int face_landmark5_postprocess(face_landmark5_out_t* pface_landmark5_result )
{
    int i = 0,j = 0,k = 0,x,y;
    int idx;
    int h32,w32,h16,w16,h8,w8;
...
void* postprocess_faceland5_dequant(nn_output *pout)
{
    ...
    for (int i = 0; i < pout->num; i++)
    {
        ...
        fl = pout->out[i].param->quant_data.dfp.fixed_point_pos;
        
        fp32_buffer[i] = (float *)malloc(size * sizeof(float));
        if (data_format == NN_BUFFER_FORMAT_UINT8 || data_format == NN_BUFFER_FORMAT_INT8)
        {
            stride = 1;
        }
        else if (data_format == NN_BUFFER_FORMAT_UINT16 || data_format == NN_BUFFER_FORMAT_INT16 || data_format == NN_BUFFER_FORMAT_FP16)
        {
            stride = 2;
        }
        else if (data_format == NN_BUFFER_FORMAT_FP32)
        {
            stride = 4;
        }
        // printf("outbuf[%d].size: %d, data_format: %d, stride: %d, zp: %d, scale: %f, fl: %d\n", i, size, data_format, stride, zp, scale, fl);
        vsi_data_to_fp32(fp32_buffer[i], pout->out[i].buf, size, scale, zp, fl, data_format);

        switch (i)
        {
            case 0:
                memcpy(prob_32,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 1:
                memcpy(bbox_32,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 2:
                memcpy(land_32,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 3:
                memcpy(prob_16,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 4:
                memcpy(bbox_16,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 5:
                memcpy(land_16,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 6:
                memcpy(prob_8,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 7:
                memcpy(bbox_8,fp32_buffer[i],size/stride*sizeof(float));
                break;
            case 8:
                memcpy(land_8,fp32_buffer[i],size/stride*sizeof(float));
                break;
            default:
                break;
        }
