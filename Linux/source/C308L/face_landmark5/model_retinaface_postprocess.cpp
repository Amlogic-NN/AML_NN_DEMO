...
void* postprocess_faceland5_dequant(nn_output *pout)
{
    ...
    for (int i = 0; i < pout->num; i++)
    {
        ...
        {
            case 0:
                memcpy(prob_32,fp32_buffer[i],size*sizeof(float));
                break;
            case 1:
                memcpy(bbox_32,fp32_buffer[i],size*sizeof(float));
                break;
            case 2:
                memcpy(land_32,fp32_buffer[i],size*sizeof(float));
                break;
            case 3:
                memcpy(prob_16,fp32_buffer[i],size*sizeof(float));
                break;
            case 4:
                memcpy(bbox_16,fp32_buffer[i],size*sizeof(float));
                break;
            case 5:
                memcpy(land_16,fp32_buffer[i],size*sizeof(float));
                break;
            case 6:
                memcpy(prob_8,fp32_buffer[i],size*sizeof(float));
                break;
            case 7:
                memcpy(bbox_8,fp32_buffer[i],size*sizeof(float));
                break;
            case 8:
                memcpy(land_8,fp32_buffer[i],size*sizeof(float));
                break;
            default:
                break;
        }
