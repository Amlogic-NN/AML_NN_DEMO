...
with open(file_name, 'r') as file:
    line = file.readline()
    BLAZE_POSE_ANCHORS = [float(value) for value in line.strip().split(',')]

#Read and preprocess the image
def preprocess_image(img_name,target_size=(224,224)):
    # Read the image
...
def test_model():
    ...
    #config.nb_path = '/data/vendor/nn/palm_detection_full_int8_from_fugui.adla'  # The adla file path in the borard: /data/nn/resnet18-v1-7_int8.adla

    config.nb_path = '/data/vendor/nn/pose_detector_int8'  # The adla file path in the borard: /data/nn/resnet18-v1-7_int8.adla
    ...
    #output_bin_path_box = './output_bin/adla_output_box.bin'
    #output_bin_path_score = './output_bin/adla_output_score.bin'


    config.input_data = processed_input # set_input from memory
    #preprocess_bin_path = './input_bin/adla_preprocess_bin'
    #save_preprocess_to_bin(processed_input, preprocess_bin_path)
    ...
    output_data_boxes = out[:(1*2254*12)].reshape((1, 2254, 12))
    print(output_data_boxes.shape)
    #save_output_to_bin(output_data_boxes,output_bin_path_box)

    output_data_scores = out[(1*2254*12):(1*2254*12+1*2254*1)].reshape((1, 2254, 1))
    print(output_data_scores.shape)
    #save_output_to_bin(output_data_scores,output_bin_path_score)
    ...
    def parse_outputs(boxes, scores, score_thresh):
        ...
        for i in range(num_boxes):
            ...
            offset_x = BLAZE_POSE_ANCHORS[4 * i + 0] * 224
            offset_y = BLAZE_POSE_ANCHORS[4 * i + 1] * 224
    ...
    cv2.imwrite('output_pose_adla_version_2_0_pose_debug_11.jpg', output_image)
